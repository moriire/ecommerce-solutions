...
<template>
  <swiper v-if="boostedProducts" :spaceBetween="30" :centeredSlides="true" :autoplay="{
    ...
    clickable: true,
  }" :navigation="true" :modules="modules" @autoplayTimeLeft="onAutoplayTimeLeft" class="mySwiper .slide-content">
    <swiper-slide v-for="(prod, index) in boostedProducts" v-bind:key="index" class="bg-danger">
      <div class="row align-items-center .mt-xs-4 .py-xs-4 .justify-content-between vh-100 .bg-success w-100">
        ...
        <div :style="`background-image: url(${prod.images[0].img}); background-repeat: no-repeat; background-position:center; height:80%`+';'"
          class=".h-100 col-lg-6 col-md-6 col-12 mb-sm-3 .order-sm-first .order-sm-0 rounded-circle" >
        </div>
      </div>
    </swiper-slide>
